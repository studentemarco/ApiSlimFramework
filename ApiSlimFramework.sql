...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `1`  AS SELECT DISTINCT `p`.`pid` AS `Pid`, `p`.`pnome` AS `Pnome` FROM (`Pezzi` `p` join `Catalogo` `c` on(`c`.`pid` = `p`.`pid`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `2`  AS SELECT `f`.`fid` AS `Fid`, `f`.`fnome` AS `Fnome` FROM `Fornitori` AS `f` WHERE !exists(select 1 from `Pezzi` `p` where !exists(select 1 from `Catalogo` `c` where `c`.`fid` = `f`.`fid` AND `c`.`pid` = `p`.`pid` limit 1) limit 1) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `3`  AS SELECT `f`.`fid` AS `Fid`, `f`.`fnome` AS `Fnome` FROM `Fornitori` AS `f` WHERE !exists(select 1 from `Pezzi` `p` where `p`.`colore` = 'rosso' AND !exists(select 1 from `Catalogo` `c` where `c`.`fid` = `f`.`fid` AND `c`.`pid` = `p`.`pid` limit 1) limit 1) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `4`  AS SELECT DISTINCT `p`.`pid` AS `Pid`, `p`.`pnome` AS `Pnome` FROM ((`Pezzi` `p` join `Catalogo` `c` on(`c`.`pid` = `p`.`pid`)) join `Fornitori` `f` on(`f`.`fid` = `c`.`fid`)) WHERE `f`.`fnome` = 'Acme' AND !exists(select 1 from `Catalogo` `c2` where `c2`.`pid` = `p`.`pid` AND `c2`.`fid` <> `c`.`fid` limit 1) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `5`  AS SELECT DISTINCT `c`.`fid` AS `Fid` FROM `Catalogo` AS `c` WHERE `c`.`costo` > (select avg(`c2`.`costo`) from `Catalogo` `c2` where `c2`.`pid` = `c`.`pid`) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `6`  AS SELECT `p`.`pid` AS `Pid`, `p`.`pnome` AS `Pnome`, `f`.`fid` AS `Fid`, `f`.`fnome` AS `Fnome` FROM ((`Catalogo` `c` join `Pezzi` `p` on(`p`.`pid` = `c`.`pid`)) join `Fornitori` `f` on(`f`.`fid` = `c`.`fid`)) WHERE `c`.`costo` = (select max(`c2`.`costo`) from `Catalogo` `c2` where `c2`.`pid` = `c`.`pid`) ORDER BY `p`.`pid` ASC, `f`.`fnome` ASC ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `7`  AS SELECT `c`.`fid` AS `Fid` FROM (`Catalogo` `c` join `Pezzi` `p` on(`p`.`pid` = `c`.`pid`)) GROUP BY `c`.`fid` HAVING sum(case when `p`.`colore` <> 'rosso' then 1 else 0 end) = 0 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `8`  AS SELECT `c`.`fid` AS `Fid` FROM (`Catalogo` `c` join `Pezzi` `p` on(`p`.`pid` = `c`.`pid`)) GROUP BY `c`.`fid` HAVING sum(`p`.`colore` = 'rosso') > 0 AND sum(`p`.`colore` = 'verde') > 0 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `9`  AS SELECT DISTINCT `c`.`fid` AS `Fid` FROM (`Catalogo` `c` join `Pezzi` `p` on(`p`.`pid` = `c`.`pid`)) WHERE `p`.`colore` in ('rosso','verde') ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`utente_phpmyadmin`@`localhost` SQL SECURITY DEFINER VIEW `10`  AS SELECT `c`.`pid` AS `Pid` FROM `Catalogo` AS `c` GROUP BY `c`.`pid` HAVING count(distinct `c`.`fid`) >= 2 ;
